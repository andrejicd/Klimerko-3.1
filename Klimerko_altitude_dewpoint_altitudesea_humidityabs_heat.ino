...
float HeatIndex;
int altitude = 376; // Altitude of the station location in meters

// ------------------------- Device -----------------------------------------------------
String         firmwareVersion         = "3.0 by andrejicd";
const char*    firmwareVersionPortal   = "<p>Firmware Version: 3.0 by andrejicd</p>";
char           klimerkoID[32];
...
char*          ALTITUDE_ASSET          = "altitude";
char*          DEWPOINT_ASSET          = "dewpoint";
char*          HUMIDITYABS_ASSET       = "humidityAbs";
char*          PRESSURESEA_ASSET       = "pressureSea";
char*          HEATINDEX_ASSET         = "HeatIndex";
...
// --------------------- SENSORS (GENERAL) ------------------------------------------------
uint8_t        dataPublishInterval     = 5;    // [MINUTES] Default sensor data sending interval
...
//movingAvg altitude(sensorAverageSamples);
...
void publishSensorData() {
  ...
  if (bmeSensorOnline) {
    ...
    JsonObject altitudeJson = doc.createNestedObject(ALTITUDE_ASSET);
    altitudeJson["value"] = avgaltitude;
    JsonObject dewpointJson = doc.createNestedObject(DEWPOINT_ASSET);
    dewpointJson["value"] = dewpoint;
    JsonObject humidityAbsJson = doc.createNestedObject(HUMIDITYABS_ASSET);
    humidityAbsJson["value"] = humidityAbs;
    JsonObject pressureSeaJson = doc.createNestedObject(PRESSURESEA_ASSET);
    pressureSeaJson["value"] = pressureSea;
    JsonObject HeatIndexJson = doc.createNestedObject(HEATINDEX_ASSET);
    HeatIndexJson["value"] = HeatIndex;
...
// Calculate reltive pressure from absolute  
  pressureSea = (((pressure)/pow((1-((float)(altitude))/44330), 5.255))) - pressureSea_cal; // Calculating the sea level pressure (Relative pressure)
...
#define hi_coeff1  -8.78469475566
#define hi_coeff2   1.61139411
#define hi_coeff3   2.33854883889
#define hi_coeff4  -0.14611605
#define hi_coeff5  -0.012308094
#define hi_coeff6  -0.0164248277778
#define hi_coeff7   0.002211732
#define hi_coeff8   0.00072546
#define hi_coeff9  -0.000003582
 ...
 // HeatIndex = ((-42.379) + (2.04901523*avgTemperature) + (10.14333127*humidity) - (0.22475541*avgTemperature*humidity) - (0.00683783*avgTemperature*avgTemperature) - (0.05481717*humidity*humidity) + (0.00122874*temperature*avgTemperature*humidity) + (0.00085282*avgTemperature*humidity*humidity) - (0.00000199*avgTemperature*avgTemperature*humidity*humidity));
  ...
  if (temperatureRaw > -100 && temperatureRaw < 150 && humidity >= 0 && humidity <= 100) {
    ...
    avgaltitude    = bme.readAltitude(1013.25);
